...
export const ShortcutsModal: React.FC = () => {
    ...
    return (
        <div className="fixed inset-0 z-[99998] flex items-center justify-center p-4" onClick={() => setOpen(false)}>
            ...
            >
                <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
                    <div className="text-sm font-bold" style={{ color: 'var(--color-text)' }}>
                        <i className="fa-solid fa-keyboard mr-2" style={{ color: 'var(--color-primary)' }} />
                        KEYBOARD SHORTCUTS
                    </div>
                    <button onClick={() => setOpen(false)} className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                ...
                <div className="overflow-y-auto max-h-[calc(80vh-50px)] p-4 space-y-4">
                    {shortcuts.map(cat => (
                        <div key={cat.category}>
                            <div className="text-[10px] font-bold tracking-wider mb-2" style={{ color: 'var(--color-primary)' }}>{cat.category}</div>
                            <div className="space-y-1.5">
                                {cat.items.map(item => (
                                    <div key={item.desc} className="flex items-center justify-between">
                                        <div className="text-xs" style={{ color: 'var(--color-text-dim)' }}>{item.desc}</div>
                                        <div className="flex gap-1">
                                            {item.keys.map(k => (
                                                <kbd key={k} className="px-1.5 py-0.5 text-[10px] rounded-sm" style={{
