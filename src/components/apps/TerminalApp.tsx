...
const COMMANDS = [
    'help', 'whoami', 'neofetch', 'ls', 'cat', 'pwd', 'uptime',
    'skills', 'projects', 'social', 'contact', 'clear', 'exit',
    'tree', 'echo', 'date', 'uname', 'free', 'df', 'top',
    'fortune', 'cowsay', 'matrix', 'theme', 'open',
];

const makeTable = (opts: { head?: string[]; colWidths?: number[]; rows: string[][]; style?: Record<string, string> }): string => {
    const t = new Table({
        head: opts.head,
...
export const TerminalApp: React.FC<TerminalAppProps> = ({ githubData }) => {
        ...
        switch (command) {
            ...
            case 'ls': {
                ...
                if (!dir || dir === '~' || dir === '/home/guest') {
                    addLine('output', '  about.md  skills/  projects/  experience.log  social/  contact.cfg  settings.conf');
            ...
            case 'cat': {
                ...
                if (file === 'about.md') {
                    ...
                    CONFIG.workExperience.forEach((exp) => {
                        addLine('output', `[${exp.startDate} – ${exp.endDate || 'PRESENT'}] ${exp.position} @ ${exp.company}`);
                    ...
                    addLines([
                        ...
                        { type: 'output', content: `font=Geist Mono` },
            ...
            case 'uptime':
                addLine('output', ` ${new Date().toLocaleTimeString()} up ${formatUptime(uptime)}, 1 user, load average: 0.42, 0.38, 0.35`);
                break;
            ...
            case 'projects': {
                ...
                addLines([
                    { type: 'output', content: '' },
                    { type: 'output', content: table },
                    { type: 'output', content: sorted.length > 20 ? `\n  ... and ${sorted.length - 20} more` : '' },
            ...
            case 'contact': {
                ...
                addLines([
                    { type: 'output', content: '' },
                    { type: 'output', content: table },
                    { type: 'output', content: '  Open the Mail Client app to send a message.' },
                    { type: 'output', content: '' },
                ]);
                break;
            }
            ...
            case 'open': {
                ...
                const appMap: Record<string, string> = {
                    terminal: 'terminal', files: 'files', filemanager: 'files',
                    editor: 'editor', about: 'editor', monitor: 'monitor',
                    skills: 'monitor', network: 'network', social: 'network',
                    mail: 'mail', contact: 'mail', settings: 'settings',
                };
                if (app && appMap[app]) {
                    addLine('success', `  Launching ${appMap[app]}...`);
                } else {
                    addLine('error', `  Available apps: terminal, files, editor, monitor, network, mail, settings`);
                }
                break;
            }
            ...
            case 'theme': {
                ...
                if (args[0] && themes.includes(args[0])) {
                    addLine('success', `  Theme changed to ${args[0]}. Open Settings to apply.`);
                } else {
                    const table = makeTable({
                        head: ['#', 'Theme'],
                        colWidths: [6, 50],
            ...
            case 'uname':
                addLine('output', '  PortfolioOS 3.0.2 portfolio-guest x86_64 React/TypeScript');
                break;
            ...
            case 'df': {
                ...
                const table = makeTable({
                    ...
                    rows: [
                        ['/dev/sda1', '500G', `${s1Used}G`, `${s1Avail}G`, `${Math.round(s1Used / 5)}%`],
                        ['/dev/sda2', '100G', `${s2Used}G`, `${s2Avail}G`, `${Math.round(s2Used)}%`],
                    ],
                });
                addLines([
                    { type: 'output', content: '' },
            ...
            case 'matrix':
                addLine('success', '  Wake up, Neo... The Matrix has you... Follow the white rabbit.');
                break;
    ...
    const lineStyle = (type: string): React.CSSProperties => {
        switch (type) {
            case 'input': return { color: 'var(--color-text)' };
            case 'error': return { color: 'var(--color-error)' };
            case 'success': return { color: 'var(--color-primary)' };
            case 'system': return { color: 'var(--color-text-dim)' };
            default: return { color: 'var(--color-text-dim)' };
        }
    };
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto space-y-0.5">
                {lines.map((line, i) => (
                    <div key={i} style={lineStyle(line.type)} className="whitespace-pre-wrap leading-relaxed">
                        {line.content}
                    </div>
                ))}
                <div ref={endRef} />
            ...
            >
                <span style={{ color: 'var(--color-primary)' }} className="mr-2 whitespace-nowrap font-bold">
                    ❯
                </span>
                <input
                    ref={inputRef}
