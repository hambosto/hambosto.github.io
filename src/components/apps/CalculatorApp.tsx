...
export const CalculatorApp: React.FC = () => {
  ...
  const evaluate = useCallback(
    (expr: string): string => {
      try {
        ...
        if (!isRadian) {
          evalExpr = evalExpr.replace(/(Math\.sin|Math\.cos|Math\.tan)\(Math\.PI\*/g, "$1(");
  ...
  const equals = useCallback(() => {
    playSound(650);
    const finalExpr = prev ? `${prev}${op}${display}` : display;
            ...
            {history.length === 0 ? (
              ...
              history
                ...
                .map((item) => (
                  ...
                  >
                    <div style={{ color: "var(--color-text-muted)" }}>{item.expression}</div>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              style={{
                backgroundColor: isRadian ? "var(--color-success)" : "var(--color-warning)",
                color: "var(--color-bg)",
              }}
            >
              {isRadian ? "RAD" : "DEG"}
                      ...
                      btn.color === "orange"
                        ...
                        : btn.color === "purple"
                          ...
                          : btn.color === "blue"
                            ? "var(--color-bg-light)"
                            : "var(--color-bg)",
                    color: btn.color === "orange" ? "var(--color-bg)" : "var(--color-text)",
            ...
            <div key={ri} className="flex gap-1 mb-1">
              {row.map((num, ni) => (
                <button
                  key={ni}
                  onClick={() =>
                    num === "00" ? (inputDigit("0"), inputDigit("0")) : inputDigit(num)
                  }
